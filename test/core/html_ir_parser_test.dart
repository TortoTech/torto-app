...
void main() {
  group('blocks', () {
    test('headings with levels and semantic style', () {
      ...
      expect((h1.inlines.single as TextRun).style.sizeScale, closeTo(1.5, 1e-9));
      final h3 = textBlock(section, 1);
      expect(h3.headingLevel, 3);
      expect((h3.inlines.single as TextRun).style.sizeScale, closeTo(1.15, 1e-9));
    });
    ...
    test('table cells degrade to paragraphs (known gap)', () {
      final section = parseSection(
          '<table><tr><th>H</th><td>a</td></tr><tr><td>b</td><td>c</td></tr></table>');
      expect(section.blocks, hasLength(4));
      final header = textBlock(section, 0);
      expect(header.kind, TextBlockKind.paragraph);
      expect((header.inlines.single as TextRun).style.bold, isTrue);
      expect(textBlock(section, 3).plainText, 'c');
    });
    ...
    test('empty blocks are dropped, image-only blocks keep the image', () {
      final section =
          parseSection('<p>   </p><p><img src="only.png"/></p>');
  ...
  group('inline styles', () {
    test('nested styles accumulate', () {
      final section =
          parseSection('<p>a <b>bold <i>both</i></b> tail</p>');
    ...
    test('links resolve root-relative and keep fragments', () {
      final section =
          parseSection('<p><a href="../ch2.xhtml#s1">go</a></p>');
  ...
  group('CSS subset', () {
    ...
    test('later rules win at equal specificity', () {
      const head = '<style>.a { color: #111111; } .b { color: #222222; }</style>';
    ...
    test('block properties: align, margins, indent, line-height', () {
      const head = '<style>p.body { text-align: center; margin: 2em 0 1em; '
  ...
  group('images', () {
    ...
    test('CSS width/max-width on img', () {
      ...
      final section =
          parseSection('<img class="f" src="a.png"/>', head: head);
      final image = section.blocks.single as ImageBlock;
  ...
  group('node ids and source ranges', () {
    test('sequential ids in document order', () {
      final section =
          parseSection('<h1>t</h1><p>a</p><p>b</p><img src="i.png"/>');
      final ids = section.blocks.map((block) => switch (block) {
    ...
    test('source ranges use UTF-16 offsets of normalized plainText', () {
      ...
      expect(block.source!.start,
          const SourceAnchor(spine: 7, node: 'n0', textOffset: 0));
      expect(block.source!.end,
    ...
    test('image blocks carry a zero-length source range', () {
      ...
      final image = section.blocks.single as ImageBlock;
      expect(image.source!.start, const SourceAnchor(spine: 3, node: 'n0', textOffset: 0));
      expect(image.source!.end, const SourceAnchor(spine: 3, node: 'n0', textOffset: 0));
    });
  });
  ...
  group('malformed input recovery', () {
    ...
    test('DOCTYPE declarations are stripped', () {
      const xhtml = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.1//EN" '
